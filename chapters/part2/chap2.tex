...
Our goal is to determine the behavior of accelerators compared to classical processor in case of heavy irregular-computationally problems.
...
\item[-] The tree traversal, called Miller's method, providing us benchmark targeting irregular memory and computation. 
\item[-]The algebraic version, called Godfrey's method, showing the performances of many-core versus multi-core architectures in respect to computationally heavy and memory irregular behaviors.  
\end{itemize}
We show the optimizations made to the regular processor algorithm to efficiently implement this application on GPU. 
We compare the two approaches for classical processor and GPU implementation. 
...
\end{tikzpicture}  
\end{center}
\caption[L(2,3) arrangement]{L(2,3) arrangement. Each pair of cube is represented by both the color and the digit in this figure.} \label{fig:lang}  
\end{figure}
...
In a more formal description we have to arrange pairs of $n$ digits from $1,...,n$ such that there is one digit between the 1s, two digits between the 2s, etc. As an example, the unique solution for $n=3$, $L(2,3)$ is the pattern $231213$ and the unique solution for $n=4$, $L(2,4)$ is $23421314$.
...
\end{tabular}
\]
\caption[Solution and times for Langford problem]{Solutions and time for Langford problem using different methods. The Miller's algorithm cannot sustain more than L(2,19) since the Godfrey's algorithm is the method we used to break new instances with L(2,27) and L(2,28).}
...
$L(2,27)$ and $L(2,28)$ have just been computed but no details were given on the time of computation~\cite{DBLP:journals/corr/AssarpourBL15}. 
...
The Miller one is based on backtracking and can be modeled as a CSP\cite{HKS00,habbas2005decomposition}; it allowed us to move the limit of explicits solutions building up to $L(2,21)$ but combinatorial explosion did not allow us to go any further. 
...
The latest known work on the Langford Problem is a GPU implementation proposed in \cite{ASS_LGF} in 2015. Unfortunately this study does not provide any performance considerations but just gives the number of solutions of $L(2,27)$ and $L(2,28)$.

\section{Miller algorithm}

The Miller's method is based on a tree traversal to be able to check all the cubes positions.
This method finds its limits because even using branch cutting algorithm to traverse the tree, the number of branches to explore stays very high. 
...
\begin{figure}[t!]
\centering
\input{\locpath/figures/tikz/part2/tree_lgf_l2_3.tex}
\caption[Search tree for L(2,3)]{Search tree for $L(2,3)$. Each level represents a pair and each branch of the level a possible position of this pair. The backtracking is used when a position is already used and the branch cannot lead to a valid solution.}
\label{fig:arbre}
\end{figure}
...
while not done do
 ...
 if successful then 
   ...
   higher pair        <- add
\end{verbatim}
\caption[Backrack algorithm for Miller's method]{Backtrack algorithm. In this case we perform the backtrack search avoiding invalid branches.}\label{backtrack}
\end{minipage}
\hspace{.05\linewidth}
\begin{minipage}[b]{0.45\linewidth}
\footnotesize
    ...
    for ...
      for pair n positions
        ...
        if final test ok then  
          count solution
\end{verbatim}
\caption[Regularized algorithm for Miller's method]{Regularized algorithm. In this case we perfom all the computation without testing if the branch will be valid or not.}\label{regularized}
\end{minipage}
\end{figure}
...
\caption[Bitwise representation of pairs positions]{ Bitwise representation of pairs positions in $L(2,3)$. 1s are positions of the pair. They are spaced by 1, 2 and 3 empty spaces, respectively.} \label{fig:comb1}
\end{minipage}
\hspace{.05\linewidth}
\begin{minipage}[b]{0.55\linewidth}
\centering
\includegraphics[width=\columnwidth]{\locpath/figures/langford/pos_lgf_v2}
\caption[Bitwise representation of the search tree]{Bitwise representation of the Langford $L(2,3)$ placement tree. The bitwise representation of the pairs are added incrementaly while going down in the tree.}\label{fig:pos_lgf}
\end{minipage}
\end{figure}
...
\caption[Test of valid position and addition of pairs]{Testing and adding position of pairs. The adding test is based on \textit{or} bitwise operation and the test on a \textit{and} bitwise operation.} \label{fig:test_et}
\end{minipage}
\hspace{.05\linewidth}
\begin{minipage}[b]{0.50\linewidth}
\centering 
\includegraphics[scale=.75]{\locpath/figures/langford/graphe_repartition}   
\caption[Client-Server distribution of tasks]{The server and client distribution of tasks. The server generates tasks up a depth. From these tasks the clients generate sub-tasks for their CPUs and GPUs.} \label{fig:parallel} 
\end{minipage}
\end{figure}
...
\begin{figure}[t!]
\centering
\includegraphics[width=\linewidth]{\locpath/figures/langford/graphe_15_9}
\caption[Grid and block size for GPU tuning]{This graph represents time as a function of grid and block size for $n=15$. This size depends on the number of registers used during the computation. That factor limits the number of blocks running simultaneously on an SMX.}
\label{f7}
\end{figure}
...
\centering
\includegraphics[width=.7\linewidth]{\locpath/figures/langford/streams.jpeg}
\caption[Number of streams tuning]{Computing time depending on the number of streams. After more than three streams there is an overhead coming from control of the streams.}
\label{fig:streams}
%\end{minipage}
%\hfill
%\begin{minipage}[b]{0.48\linewidth}
...
\begin{figure}[t!]
\centering
\includegraphics[width=.7\linewidth]{\locpath/figures/langford/graphe_cores.pdf}
\caption[Number of CPU cores used to feed the GPU]{Research of the optimal number of CPU cores used to feed the GPU. We find an optimal using both CPU and GPU with 4 cores feeding the GPU.}\label{cores_rep}
%\end{minipage}
\end{figure}
...
\begin{tabular}{l r r r}
					...
					\hline
\end{tabular}
\caption[Results for regularized method]{Results for the regularized method (seconds)}
...
\begin{tabular}{ l r r r }
					...
					\hline
\end{tabular}	
\caption[Results for the bacntrack method]{Results for the backtrack method  (seconds)}
\label{tab:result_backtrack}
\end{subfigure}
\caption[Comparison on regularized and backtrack method for Miller's algorithm]{Comparison between multi-core processors and GPUs for regularized and backtrack  on Miller's method.}
\end{table}
...
It allows us to target the specific walls with memory and computation irregularities.\\
...
\caption[Big integer representation]{Big integer representation, 64 bits words. A part of the words is used to store the data and the carries on the other half are propagate only when requested. The words are contiguous in memory for faster access.}
...
\begin{figure}[t!]
\centering 
\includegraphics[width=.7\linewidth]{\locpath/figures/langford/graphe_threads}
\caption[Number of threads per blocks for L(2,20)]{$L(2,20)$, search of the optimal number of threads per block.}
...
%\begin{subfigure}[b]{0.5\linewidth}
\centering 
\includegraphics[width=.7\linewidth]{\locpath/figures/langford/graphe_prof}
\caption[Server generation depth]{Search of the optimal server depth to generate the tasks for clients.}
...
%\begin{subfigure}[b]{0.5\linewidth}
\centering
\includegraphics[width=.7\linewidth]{\locpath/figures/langford/graphe_cpu_gpu}
\caption[Tasks repartition between CPU and GPU]{Search of the optimal distribution of tasks between CPU and GPU.}
\label{fig:graphe_rep}
%\end{subfigure}
%\caption[Server depth and client distribution of the tasks]{Search of optimzal distribution over the tree for the server depth and the client distribution among the CPU and GPU.}
\end{figure}
...
\begin{figure}[t!]
\centering
\includegraphics[scale=.4]{\locpath/figures/langford/best_effort_2}
\caption[Best-effort workflow]{This schematic represents the Best-effort workflow we used on the ROMEO supercomputer using SLURM. We fixed a manimum number of elements in the working queue to avoid overflooding the submission queue. While the tasks is not completed, it is requeued.}
...
\caption[Best-Effort tasks per 15" slots for L(2,27)]{Best-Effort tasks per 15" slots for L(2,27).}
\label{fig:graphe_15minutes_27}
\end{subfigure}
\begin{subfigure}[b]{0.5\linewidth}
\centering
\includegraphics[scale=.6]{\locpath/figures/langford/graphe_15minutes_petit_28}
\caption[Best-Effort tasks per 1 hour slots for L(2,28)]{Best-Effort tasks per 1 hour slots for L(2,28).}
\label{fig:graphe_15minutes_28}
\end{subfigure}
\caption[Best-Effort distribution of tasks for L(2,27) and L(2,28)]{Best-Effort distribution of tasks for L(2,27) and L(2,28).}
\end{figure}
...
In this first implementation we showed that the accelerators can target irregularity in memory and computation. 
The tree traversal, even with branch cutting, gives us excellent results on many-core architectures.
...
We were able to compute $L(2,20)$ with this regularized method and to get an even better computation time with the basic backtrack. 
The proposed approach exhibits that the GPU scheduler is very efficient at managing highly divergent threads.

\subsection{Godfrey's method: irregular memory + heavy computation}
We also presented the Godfrey's method which targets irregular memory accesses but also heavy computational behavior. 
The results show that the many-core version completely exceed the classical processor best performances. 
The GPU handle 65\% of the computation effort. 
In addition we beat the Langford limits using multi-GPUs. 
In order to use the ROMEO supercomputer, which is shared by a large scientific community, we have implemented a work distribution that does not affect the machine load, using a best-effort queue. 
The computation is fault-tolerant and totally asynchronous.
The utilization of this technology also shows the reliability of nowadays accelerators technologies. 
...
This is nowadays the fastest time for those problems.
...
The many-core architectures show a perfect handling of irregularity and heavy computations. 
