...
In a more formal description we have to arrange pairs of $n$ digits from $1,...,n$ such that there is one digit between the 1s, two digits between the 2s, etc. As an example, the unique solution for $n=3$, $L(2,3)$ is $231213$ and the unique solution for $n=4$, $L(2,4)$ is $23421314$.
...
The Miller one is based on backtracking and can be modeled as a CSP\cite{HKS00,habbas2005decomposition}; it allowed us to move the limit of explicits solutions building up to $L(2,21)$ but combinatorial explosion did not allow us to go further. 
...
\caption[Grid and block size for GPU tuning]{This graph represents time depending on grid and block size for $n=15$. This size depends on the number of registers used during the computation. That factor limits the number of blocks running simultaneously on an SMX.}
\label{f7}
\end{figure}
...
\begin{figure}[t!]
\centering
\includegraphics[scale=.7]{\locpath/figures/langford/best_effort}
...
Indeed, they can be used at demand for specific tasks without specific warm-up. 
