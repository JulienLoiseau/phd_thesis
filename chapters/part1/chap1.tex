...
The input and output devices can also be the same.\\
...
\hline 
Multiple Instruction (MI) & MISD & MIMD \textit{SPMD} \\
\end{tabular}
};
\draw [->,line width=.5pt] (-0.5,2) -- (5,2) node[midway, above] {Data Stream(s)};
\draw [->,line width=.5pt] (-6.2,0.5) -- (-6.2,-1.5) node[rotate=180,sloped, midway, above] {Instruction Stream(s)};
\draw [red] (2.9,-.6) ellipse (.8cm and 1.1cm) node[xshift=1.2cm,yshift=.8cm] {\textit{SIMT}};
%\draw[decoration={brace,raise=5pt},line width=1pt,decorate]
%  (5,0.4) -- node[right=6pt,yshift=5pt] {\textit{SIMT}} (5,-1.5);
\end{tikzpicture}
\caption{Flynn taxonomy for execution models completing with SPMD and SIMT models}
...
In this classification~\cite{flynn1972some} from 1972, Michael J. Flynn presents the SIMD, SISD, MISD and MIMD models represented on in table~\ref{tab:1_HPC:taxonomy_flynn} and figure~\ref{fig:1_HPC:flynn_taxonomy}.
...
Once an operation have been applied to the datum which is transfered to the next computational unit and so on. 
...
Single Program Multiple Data, SPMD is a sub-category of MIMD. 
This is the most famous parallelism way: each process execute the same program. 
...
This model have been pointed out in the Johnson taxonomy\cite{johnson1988completing}.
...
It can first define by the speed of the processor itself with the frequency defined in GHz. 
...
It can be to estimate the highest computational power of a machine. 
...
Another way to consider machine performance is to estimate the number of operations (FLOPS/IPS/IPC/OPS) regarding the power consumption. 
