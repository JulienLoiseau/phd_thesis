...
After presenting the rules of HPC and the hardware that compose the cluster, we introduce the most common ways to target those architectures and supercomputers with programming models.
...
It is based on NoRMA memories detailed in section \ref{sec:NORMA}.
...
It can be present even on node themselves: a process can be assign on a core or set of cores and the shared memory is separated between the processes. 
...
We also note that usually $g$ and $l$ are function of the number of processes involved. 
...
The \textit{Join} can be called by both parents and child. Children call join when done and the parent join by waiting until children completion.
The Fork operation increases concurrency and join decreases concurrency.
...
...
\end{lstlisting}
\caption[OpenMP 4.0 Reduction code]{OpenMP 4.0 reduction code to compute minimum. Each thread search for the smallest value in a sub-set of the overall array. The reduction from the local minimum is then handle by the API.}
...
A process can use a whole node or one or several cores on a processor.  
...
In order to hide computation steps and CPU idle time with communication a method is to use asynchrous communications with MPI.
...
\index{CUDA}
\index{NVIDIA}
The Compute Device Unified Architecture is the API develop in C/C++ by NVIDIA to target its GPGPUs. 
...
The ISA on this architecture is simple and allows to work directly in assembly for very fine grain optimizations. 
...
The idea of OpenCL is great by relying on the vendors' wrapper.
...
Those questions are still unanswered but many tests prove that OpenCL can be as comparable as CUDA but rarely better\cite{karimi2010performance,fang2011comprehensive}. 
...
In real application the only way to really know what will be the behavior of a supercomputer is to try, test real codes on it. 
...
\subsection{TOP500}
\index{TOP500}
The most famous list is certainly the TOP500\footnote{http://www.top500.org}. 
...
This list is based on the LINPACK\cite{dongarra1994top500} benchmark introduced by Jack J. Dongarra.\index{LINPACK}
This benchmark relies on solving dense system of linear equations. 
...
\index{PEZY}
\index{NVIDIA}
In conjunction of the TOP500, the Green500\footnote{https://www.top500.org/green500/} list focuses on the energy consumption of supercomputers. 
...
\subsection{GRAPH500}
\index{GRAPH500}
The GRAPH500\footnote{https://www.graph500.org/} list\cite{murphy2010introducing} focus on a benchmark featuring irregular memory accesses, and communications.
...
The aim of the benchmark is to generate a huge graph to fill all the maximum memory on the machine and then operate either:
\begin{description}
...
This study intents to provide a specific metrics representing computation and communication walls with highly irregular applications. 
