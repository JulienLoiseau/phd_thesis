...
\end{tikzpicture}
\end{center}
\caption[Vector processor computation, example of addition]{Example of a vectorized processor with a 128 bits wide bus. It shows the addition of 4 integers using serial and vectorized approach.}
...
\end{tikzpicture}
\caption[Architecture of cache memory]{The cache memory technology on three levels: L1, L2 and L3. The closest memory to the CPU is the fastest but also the most expensive.}
...
This device can be considered as a Host or Device/Accelerator machine (for the earliest versions). 
...
The main advantage of this architecture compared to GPGPUs is the x86 compatibility of the embedded cores and the fact this device can either be bootable or used as an accelerator. 
...
\begin{center}
% classical processor
\input{figures/tikz/part1/multi_many_cores.tex}
\end{center}
...
\index{NVIDIA}
The NVIDIA company was founded in April 1993 in Santa Clara, California by three persons, one being the current CEO, Jensen Huang.
...
This knowledge on the architectures and hardware is essential to be able to reach the highest-level optimization and thus the best performance for a dedicated machine. 
