...
\section{Architecture improvements and parallel capabilities}
...
Plenty of other optimizations were added to the cores
...
This behavior is presented with a 16 bytes vector machine (4 integer of 4 bytes = 128 bits bus) on figure~\ref{fig:2_HARD:vector}.
...
\section{Clusters}
Those first technologies allows the constructor to build machines composed of multiple single core processors. 
They are now composed of thousands of nodes interconnected with the best...
In order to connect them an interconnect technology is required. 
...
For this purpose we present the latest architectures with many-core processors and the most famous vendors. 
We start from the most present in HPC world, the company Intel. 
...
\section{Remarkable supercomputers}
