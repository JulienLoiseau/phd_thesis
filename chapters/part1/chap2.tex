...
We choose to present the architectures in a chronological order following the models presented in the previous chapter with: SISD, MIMD and SIMD/SIMT.
We also present the optimizations of technologies through parallelism and memories.


\section{Architectural improvements and parallel capabilities}
...
The first processors around 1970s were built using a single computation core like describe in the Von Neumann model. 
In order to reach more performances the first factor was to reach higher frequency in the computational cores reaching up to 4-5GHz for best nowadays processors. 
...
\subsubsection{Pipeline, vectorization} Processors allows the instructions to be executed at the same time in a SIMD manner. 
