...
The previous part describes the method we want to implement and presents the wall in the smoothed particle hydrodynamics and gravitation computation. 
...
%
\begin{algorithm}
\caption{SPH implementation}\label{alg:sph}
\begin{algorithmic}[1]
...
The SPH seems to be a very good candidate to benchmark the binary, quad and oct tree topology. 
...
FleCSPH is base on the tree topology of FleCSI and will follow the same structure define in FleCI. 
...
The continuous integration is ensured by using Travis based on Docker provided in the Docker folder. 
In addition to Travis for the unit tests we use use tools like CodeCov\footnote{\url{http://codecov.io}} for the code coverage and SonarQube\footnote{\url{http://sonarqube.org/}} for the quality gates. 
...
%The FleCSPH code for this version is based on synchronous communication. 
%This reduces the scalability performances. 


\section{Distributed SPH on hybrid architectures}
We constructed an efficient and reliable SPH code working on multi-CPU clusters. 
...
The fluid simulations are presented on figure~\ref{}.

The final test of our study is a binary neutron stars merging simulation. 

\section{Conclusion}
In this part we presented the FleCSPH framework base on FleCSI. 
We gave details on the code structure and the strategies taken for the multi-CPU version. 
We presented the tree traversal, divided in two steps in order to generated better interaction lists for particles. 

The results of the multi-CPU code are presented and shows good results in both side, physics and computer science. 

In the last section we presented the multi-GPU code.
