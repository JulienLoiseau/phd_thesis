...
In order to compute the gravitation for a group of particles in the domain we steps are: \\
...
Then to go down in the tree and apply the gravitation to locals and then particles in L2L and L2P, the values are use with equation~\ref{eq:taylor_series}.\\
...
The irregular behavior is also present for the choice of the multipole to consider during the M2L step.
...
We choose a 4 mains examples in this purpose, the Sod shock tube, the Sedov blast wave, fluid flow simulation and astrophysical examples like binary neutron stars coalescence.
